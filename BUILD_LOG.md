...
### Live data JSON files missing on deploy (May 2026)
- **Symptom:** Build fails with `Could not resolve "../data/live/season.json" from "src/pages/conditions.astro"`
- **Root cause:** Pages imported JSON files from `src/data/live/` which are generated by `scripts/fetch-live-data.mjs`. The build script only ran `astro build` — it did not run the fetch script first. On DigitalOcean (fresh checkout), the JSON files didn't exist, so Vite's import resolution failed at build time, even though the imports were wrapped in try/catch.
- **Why try/catch didn't help:** Vite resolves `import()` paths statically during the build step. If the file doesn't exist on disk, Vite throws before the runtime try/catch executes.
- **Fix (two parts):**
  1. Changed build script in `package.json` from `"astro build && ..."` to `"node scripts/fetch-live-data.mjs && astro build && ..."` so data is fetched before Astro builds.
  2. Committed seed JSON files to git so they always exist on fresh checkout, even if the fetch script fails.
- **Lesson:** Never add build-time imports to files that don't exist in git. If a file is generated at build time, either (a) run the generator BEFORE the build in the same script, or (b) commit seed/fallback versions to git. Never rely on try/catch around `import()` in Astro — Vite resolves imports statically.
